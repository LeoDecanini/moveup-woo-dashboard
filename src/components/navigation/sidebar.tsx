...
const menuOptions = [
  { text: "Productos", link: "/create-product" }
];
...
/*   useEffect(() => {
  ...
  return (
    ...
    <nav
      className={`hidden min-[900px]:block fixed top-0 z-40 left-0 h-full bg-secondary border-r border-black/30 transition-all duration-300 ${
        !isLocked ? "w-64" : "w-16"
      } overflow-hidden`}
    >
    ...
    <main
      className={`w-full flex transition-all duration-300 flex-col items-center justify-center  ${
        !isLocked ? "min-[900px]:pl-64" : "min-[900px]:pl-16"
      }
      `}
    >
      <div
        className={`w-full h-full transition-all duration-300 max-w-[1500px]`}
      >
        {/* <NavBar /> */}

        <div className="pt-5 transition-all duration-300 min-h-[calc(100svh-64px)]">
          {children}
        </div>
      </div>
    </main>
